...
public interface accountRepository extends JpaRepository<Account, Long> {
	...
	@Query(value = "select * from TBLACCOUNT "
			+ "where (ACCOUNT_CODE like ?1 or  ACCOUNT_DATE like ?1 or ACCOUNT_NAME like ?1 or ACCOUNT_DESCRIPTION like ?1  or ACCOUNTPARENT_ID like ?1) "
	...
	@Query(value = "select * from TBLACCOUNT " 
			...
			+ "and ISACTIVE='Y'", nativeQuery = true)
	List<Account> findByAdvancedSearch(Long generalratetype_ID, Long accounttype_ID, Long classflowratetype_ID);
	...
	@Query(value = "select * from TBLACCOUNT " 
			...
			+ "and GENERALRATETYPE_ID LIKE CASE WHEN ?1 = 0 THEN GENERALRATETYPE_ID ELSE ?1 END "
			, nativeQuery = true)
	List<Account> findAllByAdvancedSearch(Long generalratetype_ID, Long accounttype_ID, Long classflowratetype_ID);
